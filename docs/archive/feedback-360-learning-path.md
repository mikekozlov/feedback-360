# feedback-360: learning path for rebuilding in a separate folder

## Что это за learning path

Этот документ не заменяет существующие rebuild-guides и не повторяет их постранично. Его задача уже другая: дать вам **self-paced curriculum**, по которому вы сможете **пересобрать подход проекта в отдельной папке**, постепенно понять его и сравнивать себя с оригиналом в правильных точках.

Три companion-документа теперь играют разные роли:

- [`feedback-360-faithful-rebuild-guide.md`](./feedback-360-faithful-rebuild-guide.md)
  - conceptual reverse-engineering;
  - отвечает на вопрос: **что это за подход и почему он устроен именно так**.
- [`feedback-360-step-by-step-rebuild.md`](./feedback-360-step-by-step-rebuild.md)
  - technical reconstruction manual;
  - отвечает на вопрос: **как технически повторять слои и шаги**.
- `feedback-360-learning-path.md`
  - curriculum layer;
  - отвечает на вопрос: **как учиться на этом rebuild, не превращая его в бездумное копирование**.

### Рабочие термины

В этом документе используются два фиксированных пути:

- `REFERENCE_ROOT = C:\Users\mykha\Projects\feedback-360`
- `REBUILD_ROOT = C:\Users\mykha\Projects\feedback-360-lab`

Смысл простой:

- `REFERENCE_ROOT` — оригинальный repo, который вы **читаете и изучаете**, но не используете как место для rebuild-практики;
- `REBUILD_ROOT` — ваш отдельный учебный repo, в котором вы **строите собственную версию подхода**.

Важно: learning path предполагает, что оригинальный `feedback-360` остаётся reference system. Вы не мутируете его “в процессе обучения”, а используете как живой образец.

## Правила обучения

Чтобы этот путь действительно научил подходу, а не только повторению файлов, соблюдайте следующие правила.

1. Не копируйте готовые файлы из `REFERENCE_ROOT` до checkpoint.
   - Сначала попробуйте восстановить идею и структуру сами.
2. Сначала читайте контекст, потом кодите.
   - Если модуль требует сначала открыть `ADR`, `spec` или companion-guide, делайте это до начала работы в `REBUILD_ROOT`.
3. Сравнивайте себя с оригиналом только в контрольных точках.
   - Не держите оригинальный код открытым как “шпаргалку на каждый шаг”.
4. Не пытайтесь сразу “догнать весь проект”.
   - В этом подходе важна последовательность: knowledge structure -> deterministic delivery path -> domain slices -> UI -> hardening -> refactor -> mature traceability.
5. Не начинайте с UI.
   - Если у вас в `REBUILD_ROOT` раньше времени появляются React-экраны, значит вы ушли от метода.
6. Не смешивайте `spec`, `plans` и `adr`.
   - Если не можете ответить, где у вас WHAT, где WHY, а где delivery slicing, структура уже поплыла.
7. Не прячьте business logic в route handlers, CLI или UI.
   - Любая такая “маленькая валидация для удобства” быстро ломает thin-layer architecture.
8. Не превращайте `shared` в свалку.
   - Если модуль имеет явного owner по feature area, он не должен жить в `shared`.
9. Не считайте модуль завершённым без доказательства.
   - `Done when` в этом документе важнее внутреннего ощущения “ну, вроде сделал”.

## Маршрут обучения

Ниже маршрут разбит на self-paced модули. Вы проходите их в своём темпе, но **строго по порядку**.

Каждый модуль имеет один и тот же учебный контракт:
...
- `Artifacts` — что сохранить как доказательство и как учебную заметку.
...
## Модули learning path

## Module 0: Понять методологию `docs-first + Memory Bank + vertical slices`
...
- Создать в корне `REBUILD_ROOT` только один личный файл заметок, например `LEARNING_NOTES.md`.
...
- [`docs/feedback-360-faithful-rebuild-guide.md`](./feedback-360-faithful-rebuild-guide.md)
- [`docs/feedback-360-step-by-step-rebuild.md`](./feedback-360-step-by-step-rebuild.md): `Глава 0`
...
- Завести минимальные root scripts:
  - `lint`
  - `typecheck`
  - `test`
  - `checks`
- Создать верхнеуровневые каталоги:
  - `apps/`
  - `packages/`
  - `.memory-bank/`
...
- [`docs/feedback-360-step-by-step-rebuild.md`](./feedback-360-step-by-step-rebuild.md): `Глава 1`
- [`../package.json`](../package.json)
- [`../pnpm-workspace.yaml`](../pnpm-workspace.yaml)
- [`../.memory-bank/plans/epics/EP-000-foundation/features/index.md`](../.memory-bank/plans/epics/EP-000-foundation/features/index.md)
...
## Module 2: Собрать documentation skeleton и базовые rules
...
- Создать минимальный skeleton для:
  - `.memory-bank/spec/`
  - `.memory-bank/plans/`
  - `.memory-bank/adr/`
  - `.memory-bank/mbb/`
- Добавить root index и индексы ключевых разделов.
- Добавить минимум такие документы:
  - repo structure;
...
- [`docs/feedback-360-step-by-step-rebuild.md`](./feedback-360-step-by-step-rebuild.md): `Глава 2`
- [`../.memory-bank/index.md`](../.memory-bank/index.md)
...
- Сравните not wording, а structure and purpose.
...
## Module 3: Сделать DB baseline и seed runner
...
- Выбрать и зафиксировать DB layer так, чтобы он был совместим с дальнейшим rebuild path.
- Добавить:
...
- [`docs/feedback-360-step-by-step-rebuild.md`](./feedback-360-step-by-step-rebuild.md): `Глава 3`
- [`../.memory-bank/plans/epics/EP-000-foundation/features/FT-0002-db-migrations-baseline/index.md`](../.memory-bank/plans/epics/EP-000-foundation/features/FT-0002-db-migrations-baseline/index.md)
- [`../.memory-bank/plans/epics/EP-000-foundation/features/FT-0003-seed-runner-handles/index.md`](../.memory-bank/plans/epics/EP-000-foundation/features/FT-0003-seed-runner-handles/index.md)
...
## Module 4: Реализовать `api-contract + core + client + CLI`
...
Не пытайтесь сразу покрыть много domain behavior. Одна работающая operation важнее десяти абстракций.

### Read in REFERENCE_ROOT

- [`docs/feedback-360-step-by-step-rebuild.md`](./feedback-360-step-by-step-rebuild.md): `Главы 4-7`
- [`../.memory-bank/adr/0001-core-client-cli-first.md`](../.memory-bank/adr/0001-core-client-cli-first.md)
...
## Module 5: Сделать первые domain slices
...
Не пытайтесь сразу повторить весь HR domain. На этом модуле важнее освоить ритм slice delivery.

### Read in REFERENCE_ROOT

- [`../.memory-bank/mbb/templates/feature.md`](../.memory-bank/mbb/templates/feature.md)
- [`../.memory-bank/plans/implementation-playbook.md`](../.memory-bank/plans/implementation-playbook.md)
- [`../.memory-bank/plans/epics/EP-001-core-contract-client-cli/features/index.md`](../.memory-bank/plans/epics/EP-001-core-contract-client-cli/features/index.md)
...
- Вы можете показать, где в каждом slice находится:
  - спецификация;
  - код;
  - тест;
  - evidence.
...
## Module 6: Добавить thin web layer
...
- Реализовать один-два route handlers или экранных потока поверх уже существующих operations.
...
- [`docs/feedback-360-step-by-step-rebuild.md`](./feedback-360-step-by-step-rebuild.md): `Глава 12`
...
- relevant UI/test docs from `.memory-bank/spec/ui/`
...
## Module 7: Пройти hardening и quality gates
...
- Добавить/усилить:
  ...
  - typecheck;
  - tests;
  - basic build step if relevant;
  - minimal acceptance evidence discipline.
- Описать у себя, что считается completed slice.

### Read in REFERENCE_ROOT

- [`../.memory-bank/spec/engineering/testing-standards.md`](../.memory-bank/spec/engineering/testing-standards.md)
- [`../.memory-bank/spec/engineering/delivery-standards.md`](../.memory-bank/spec/engineering/delivery-standards.md)
- [`../.memory-bank/plans/verification-matrix.md`](../.memory-bank/plans/verification-matrix.md)
...
- В `REBUILD_ROOT` есть хотя бы простая, но формальная delivery discipline.
...
## Module 8: Выполнить feature-area refactor
...
- [`../.memory-bank/spec/project/feature-area-boundaries.md`](../.memory-bank/spec/project/feature-area-boundaries.md)
- [`../.memory-bank/plans/epics/EP-014-feature-area-slices-refactor/index.md`](../.memory-bank/plans/epics/EP-014-feature-area-slices-refactor/index.md)
...
## Module 9: Понять XE scenarios, guides и UI traceability как позднюю волну зрелости

### Goal

Понять, что зрелый проект в этом подходе — это не только код и tests, но и сценарный runtime, операторские guides и строгая UI traceability.
...
- [`../.memory-bank/plans/epics/EP-020-cross-epic-scenarios/index.md`](../.memory-bank/plans/epics/EP-020-cross-epic-scenarios/index.md)
- [`../.memory-bank/spec/ui/screen-registry.md`](../.memory-bank/spec/ui/screen-registry.md)
- [`../.memory-bank/spec/ui/test-id-registry.md`](../.memory-bank/spec/ui/test-id-registry.md)
- [`../.memory-bank/spec/ui/pom/index.md`](../.memory-bank/spec/ui/pom/index.md)
- `git ranges`:
  - `c85a165..45496ff`
  - `9f6f5a3..a8bc502`
...
1. Сначала сравните **идею**.
   - Для этого используйте `faithful-rebuild-guide`.
2. Потом сравните **практический способ сборки**.
   - Для этого используйте `step-by-step-rebuild`.
...
## Признаки, что вы действительно усвоили подход

Вы усвоили подход, если можете объяснить следующее без подсказок:

1. Почему порядок именно такой:
   - docs -> foundation -> contract/core/client/cli -> domain slices -> UI -> hardening -> feature-area refactor -> XE/traceability.
...
5. Почему feature-area refactor был выделен в отдельную волну, а не размазан по всем фичам.
...
Слабый признак усвоения:

- “Я собрал похожую структуру папок”.

Сильный признак усвоения:

- “Я могу обосновать архитектурный порядок, pressure points и правила завершения работы”.

## Recommended rhythm for self-paced study
...
Используйте `REFERENCE_ROOT` как живую reference system, а `REBUILD_ROOT` как учебную лабораторию.

Ваша цель не в том, чтобы как можно быстрее получить второй `feedback-360`. Ваша цель — научиться воспроизводить такой же тип engineering evolution:

- сначала структурировать знание;
- потом строить deterministic path;
- потом выращивать behavior slices;
- потом добавлять UI;
- потом укреплять delivery discipline;
- потом делать refactor только тогда, когда для него созрел pressure.

Если вы научитесь именно этому порядку, то сможете воспроизводить не этот один проект, а сам подход в других репозиториях.
